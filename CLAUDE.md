...
6a. **DEPLOY IS `cd site && npx vercel --prod --archive=tgz`.** The Vercel project is **`migne`** and it is linked at **`site/`**, which is also the deploy root (`vercel.json` rewrites live there). The repo root is *separately* linked to a stray project named `patrologia` that serves nothing — running `npx vercel --prod` from the repo root deploys **the wrong project**, succeeds, prints a healthy-looking URL, and changes nothing on migne.app. Done by mistake 2026-07-29. `--archive=tgz` is required either way: the tree exceeds Vercel's 15,000-file upload cap.
