...
6a. **SHIP IS THREE COMMANDS, IN THIS ORDER: `node scripts/build-site.mjs <idno>` → `node scripts/preflight.mjs` (must exit 0) → `cd site && npx vercel --prod --archive=tgz --scope wilson-pruitts-projects`.** ⛔ **`build-site.mjs` exists because shipping 8950 on 2026-09-05 meant remembering NINE builders and three were run** — the deploy went out with the work live and correct at its own URL and **absent from `/glossa`**, and preflight passed 6 of 6 over it, because it checked that a built page EXISTS and never that the indexes listing it were regenerated. It was caught by grepping the DEPLOYED html for the new work's slug, which nobody remembers to do. Preflight now also carries an **index-freshness** check (no index older than the newest work page), verified by making it fail. ⚑ Still smoke-test the real domain after deploying — the alias, not the deployment URL.
6b. **DEPLOY IS `cd site && npx vercel --prod --archive=tgz --scope wilson-pruitts-projects`.** ⚑ **The `--scope` is not optional and its absence does NOT fail like a missing flag — it fails as `"Not authorized"`, which reads exactly like a wrong-account or bad-token error** (2026-09-05, shipping 8967). The standing diagnostic is `vercel teams ls`, and the note attached to it says that if the linked org is missing from the list no `--scope` will help. **The converse is what bit here: the org WAS listed, `vercel project ls` showed `migne` bound to migne.app, `vercel whoami` gave the right account — and the deploy still returned "Not authorized" until `--scope` was passed explicitly.** So the message has at least two causes. **Try `--scope` first; only then suspect the account.** The Vercel project is **`migne`** and it is linked at **`site/`**, which is also the deploy root (`vercel.json` rewrites live there). `--archive=tgz` is required: the tree exceeds Vercel's 15,000-file upload cap. `migne.app` + `www.migne.app` are bound to that project and no other. **Do not link the repo root to a Vercel project.** It was linked to a stray `patrologia` project that served nothing, was Git-connected (so every push burned a build producing 404s), and silently absorbed a `--prod` deploy meant for the site — it printed a healthy URL and changed nothing on migne.app. Project deleted and the root link removed 2026-07-29; if a root `.vercel/` ever reappears, delete it rather than deploying from there.
