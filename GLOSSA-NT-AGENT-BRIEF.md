...
⛔⛔ **AND THE "DIRECTION RULE" THIS BOX ONCE GAVE IS ITSELF WITHDRAWN — 2026-09-05, same day.**
It said: ink can turn one letter into another but cannot put a word on the page that is not there,
so a claim that *the plate has MORE than our twin* is safe on one witness. **That rule was written
around the strongest such claim in 8950 — a dropped `non` at 0141B — and the claim is false.**
Gallica f71 (corners 141/142) prints:

    Vel aquam praebuit, quia in eo quod credidit, vacua non remansit.

**There is no `non` after `quia`**, and `quia in eo` is closed up mid-line at normal letter spacing.
⚑ **The sentence's only `non` is in the NEXT CLAUSE** — *vacua **non** remansit*. The plate read did
not hallucinate a word out of ink; **it attributed a word that really is on the page to the wrong
slot, in a sentence whose argument wanted a negative there.** That is 7a″ conformation — the defect
this project hunts in translation — operating inside a PLATE READ, where nothing downstream checks
for it.

⭐ **So the corrected rule is the blunt one: NO claim about Migne's type stands on one witness, in
either direction.** Not a changed letter, not a changed digit, not a word said to be missing. What
actually survived two witnesses in 8950 was a narrow, checkable set — **lost Hebrew** (whole
non-Latin words, unmistakable and never generated by ink), **Migne's line-head `«` repeats**, and
**defects in our own transcription against a clean plate**. Everything else fell.
