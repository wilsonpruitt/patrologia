...
**Prep done 2026-09-25:** re-chunk byte-identical (81 chunks, 109,419 w). Scan `raw/scans/pl195/patrologiaecursu0195mign.pdf` (archive `patrologiaecursu0195mign`; ⚠ volumes.json's archive item for PL 195 is an AUGUSTINE volume). **PDF page = floor((col+13)/2)**, pp. 507–645 = 139 pages. Migne prints a SPARSE numbered foot layer ((3) *Deest aliquid* p.613, (4) *Locus obscurus* p.638) absent from our Latin: stints recover it inline into `data/briefs/11512-NOTES-<first>.tsv` → merge into `data/plate-notes/11512.tsv`, re-chunk, set coverage. Split pages at Book IV (p.628) and the epilogue (p.642) make anchors run 1244C→[1243]→1243D: CORRECT, not a defect. Bold numbers = Gravius's 1630 edition pages, kept verbatim. Lemma inventory 2,849 spans, split into 16 files `data/briefs/11512-lemmata-<first>.txt`.
**Brief:** `data/briefs/11512-STINT-BRIEF.md`. 16 stints of 5 chunks (last = 0075–0080), 3 at a time. Estimate quoted: ~6M translate + ~3M blind read + ~0.5M merge ≈ 9.5M, Opus.
**Rounds:** R1 T01–T03 (0000–0014) launched 2026-09-25. Check disk (English chunks, cruces-<first>.md, PLATE-READS, NOTES tsv) for each range before resuming anything.
**Merge follow-ups logged from stints (do at merge):** (1) T03: p.523 corners unread; 1040D *a regione* vs Vulgate *e regione* unverified (scan A blacks out one-letter words; use scan B `raw/scans/pl195/patrologiaecur195mign.pdf`, page = floor((col+9)/2)). (2) T02: after TEI patch 1017D (*sibi porrigi* → plate *sibipo rrigi*) lands, add the owed `[sic:]` in English 0005–0009; 1024B *fecunditata* final letter smudged, check on scan B. (3) T01: 1013A TEI wraps Wolbero's running parenthesis in a <note> (11085 class); English carries it as [nt:] until patched. **Footnote numbering restarts per page** ((1) on each). ⚠ Brief v1 wrongly said numbered [cn:] get English twins; corrected same day (numbered = Latin-only, verify 9a).
(4) T04: 1067–68 Migne prints a full-width DIAGRAM (*56 FIGURA*); TEI keeps only the caption; English lists labels under an [ed:], Latin labels in cruces-0015 — decide at merge whether the labels go into the Latin as a TEI patch (check how earlier works handled figures). Migne labels both Ct 1:8 and 1:9 *VERS. 9* (kept as printed).
(5) T05 reconciliation terms: *accubitus* "reclining" (gloss plays on accumbere/reclinatorium), *murenulae* kept italic Latin, *monilia* "necklaces" (check T04 Ct 1:9), *proportior* "greater in proportion" (also T04 0018).
(6) Normalize the 5 early `VERS. n.` → `VERSE n.` in 0005, 0006, 0008, 0027 (ruled mid-run). T06: `clementine-flat.txt` prints Ct 2:5 *Fulcite in me* — check the comparand file itself.
(7) Latin carries an EMPTY head `## .` (first in 0020/0021, "(cont.)" through 0026): a TEI artifact — read the plate where it starts and patch the head text. T09: once 1148C *colnmbarum* patch lands, add the owed `[sic:]` in 0044. Head style mirrors the Latin form for form (CAPUT PRIMUM→CHAPTER ONE, CAP. III→CHAP. III.-- *Continuation.*): keep, no normalization.
(8) Bride/Bridegroom capitalization: rule set mid-run = follow Migne's case per instance (19 *Sponsus* / 99 *sponsus*; 2 *Sponsa* / 96 *sponsa*). Chunks 0000–0044 are MIXED (60 "Bridegroom" vs 19 lower): align at merge against the Latin, instance by instance.
(9) Pattern 3 sweep: 21× "divine Scripture" (for *divina Scriptura*) in 0000–0054 → "Holy Scripture". T11: 1184C *montis*→*mentis* patch is a close read, re-check at patch pass.
(10) T12: after 1203D *candidatio*→plate *candidutio* patch lands, add owed `[sic:]` in 0059.
(11) ⛔ **c/e RE-CHECK OWED (T10 finding): scan A cannot distinguish c from e.** Every `[sic:]` and TEI patch fired on scan A ALONE in T01–T09 (and any later without a B-read) that turns on c/e or another ink-filled letter must be re-read on scan B at 600 dpi before the plate gate. T10 withdrew *crubescenti*, *eor* that way. T12 *pereunctantium* is B-confirmed. T10: 1169C `[cj:]` *scriptum*→*Christum* is a judgment call — adjudicate (crux vs cj). Collation-file errors in clementine-flat.txt: Ct 4:4 *pendant*, 1 Rg 15:23 *nole*, Ct 2:5 *Fulcite in me*. T10 follow-up: after 1159D *illud*→*illua* patch, owed `[sic:]` in 0047.
(12) FOOTNOTE NUMBERING is irregular — do not "fix" it: p.509 (1) and p.510 (1) (T01, front matter), p.613 lone (3) keyed at 1213A after *qui imperfecti demonstrant* (T13; no (1)/(2) anywhere on pp.600–619), p.638 per T16. Every page was read by its stint, so the set is exhaustive; record the numbers exactly as printed. OCR confuses 1/4 on note numbers. T13 follow-ups: after 1209A *coelestis*→*coelestus* patch, owed `[sic:]` in English; 1214B *misericord*i*am* markup patch; 1215D *neeessariam* crux only (ambiguous on both scans). T14: 1231C *dil?tionis* unresolved (letter missing on both copies), English keeps "love"; 1233C VERS. 3→5 patch.
**Measured:** T01 333K, T02 400K, T03 326K for 5 chunks each (~71K/chunk).
**MERGE PROGRESS (2026-09-25):** ✅ all 16 stints done, 81 chunks verify OK, committed `2cc8afef` (translation ≈5.97M tok). ✅ Migne's 4 notes → `data/plate-notes/11512.tsv` (anchors carry the "(n)" key), coverage.json `full` with `sequence_gaps_documented: [2]`, re-chunk injects 4/4, Latin byte-identical to hand placement. ✅ Pattern 3 sweep (18× divine→Holy Scripture). ✅ VERS. already normalized by stints. ✅ Bride/Bridegroom aligned to Migne's case (42 lowered; no reverse cases). ✅ plate reads merged into data/plate-reads.json works.11512 (208 reads, pp.507–645 complete); plate-gate OK. ⚠ **B-CHECK STALL LESSON (2026-09-26):** all three first checkers (B1–B3) died on 600 s stream-watchdog stalls, twice each on resume. Cause: page images accumulating in context (B3 held 58 crops). None had written anything, because the brief said "one file at the end". Remedy that worked: fresh agents on EXPLICIT site lists (`11512-BCHECK-SITES-C{1,2,3}.tsv`), header written first, ONE tight crop per site, append the row, delete the PNG. **Brief every plate-image agent this way from now on (blind readers too).** B1 wrote 51 rows (`11512-BCHECK-0000.tsv`), B2 53 (`-0020.tsv`, labels its patch rows "tei"), B3 none. Disputed: 1152D *crubescenti* (T10: B reads *erubescenti*; B2 REJECTED) → C3 adjudicates.
✅ **B-CHECK DONE + APPLIED 2026-09-26 (commit `1b2d8a03`):** 105 [sic:] stand / 1 withdrawn (1191B ours); 70 TEI patches in `data/tei-patches/11512.json` (4 rejected, 5 undecidable → cruces); 13 owed [sic:] fired; phantom `## .` head removed (1070D); merged `src/english/11512/cruces.md` (work-wide conventions + all adjudications). verify OK, plate-gate OK. ⏳ **BLIND READ** running: brief `data/briefs/11512-BLIND-READ-BRIEF.md`, 12 readers R01–R12 (7 chunks each; R11 0070–0075, R12 0076–0080), 3 at a time, reports `data/briefs/11512-BLIND-READ-R##.md`. R01–R03 launched. THEN: adjudicate + apply → polarity-record → Wolbero bio (author-bios.json) + about (work-about.json) → englishState ours (workStatus stays none → First) → build-site 11512 → preflight → STAGE; deploy = Wilson's OK.
(superseded) ⏳ B-CHECK of all 105 [sic:] + 76 TEI patches on scan B at 600 dpi (brief `data/briefs/11512-BCHECK-BRIEF.md`, lists `11512-SIC-LIST.tsv` / `11512-TEI-PATCH-LIST.tsv`, outputs `11512-BCHECK-{0000,0020,0050}.tsv`). NEXT: apply B-check verdicts (withdraw OURS/UNDECIDABLE sics, drop REJECTED patches) → write `data/tei-patches/11512.json` → re-chunk → fire owed [sic:] (1017D, 1039B vinnm, 1148C, 1159D, 1203D, 1209A…) → figure 1067–68 + empty `## .` head → merge cruces (work-wide conventions section) → blind read.
**After all 16:** merge NOTES tsvs → plate-notes + re-chunk (Latin gets [cn:]) · apply TEI patches from the cruces files · merge cruces + work-wide conventions section · merge PLATE-READS into data/plate-reads.json · verify-english · blind polarity read (cruces withheld) · polarity-record · Wolbero bio + about · plate-gate · englishState ours (workStatus stays `none` → First) · build-site 11512 → preflight → STAGE; deploy needs Wilson's OK.


## ▶▶▶▶▶▶▶▶▶▶▶▶▶▶▶ START HERE (2026-09-24) — INNOCENT I, PL 20 (7089 · 7090 · 7091) · ✅ LIVE ON migne.app (deployed 2026-09-24, `migne-3olovguck`, commit `000cfc85`, pushed) · NOTHING OWED BUT THE HELD ITEMS

**Built and staged, NOT deployed, NOT committed.** `build-site.mjs 7089 7090 7091` ran, `scan-raw-markers` is clean, `preflight` exits 0 (7 checks; ratchet ceiling 928 not exceeded). Badges on the built pages: 7089 **First**, 7090 **New** (`priorEnglishKnown`), 7091 **First**. 7091's byline is **Ps.-Innocent I**, via the NEW `data/author-overrides.json` (keyed by workIdno, applied in `scripts/chunk-work.mjs` before `chunkWork`; `authorIdno` untouched) plus a `Pseudo-Innocentius I` entry in `data/author-bios.json`. `englishState: "ours"` set on works 533/534/535 (`workStatus` untouched). About texts in `data/work-about.json` (7089 carries Labbe's "not improbable conjecture" doubt; 7090 says Coustant's notes are in with twins and single letters exist in older English; 7091 says forged per Coustant and each block translated from its own language). Landing RECENT list has the three.
**Deploy (needs Wilson's OK):** `cd site && npx vercel --prod --archive=tgz --scope wilson-pruitts-projects`, then smoke-test `/`, `/pl/20/epistolae-et-decreta/`, `/pl/20/decreta/`, `/pl/20/epistolae/` and the three `/cruces/pl/20/…` pages on the ALIAS. Then commit (local to master is fine; push needs the usual OK).

**The passes, and what each cost:** 7 Opus translating stints ≈ 2.8M subagent tokens (36 chunks, all 82 pages read once at the plate) · 7 Opus blind readers ≈ 1.39M · adjudication fork ≈ 0.29M · merge fork ≈ 0.21M. Total ≈ 4.7M against the 5.4M quoted.
**What the passes found:** the Phase-1 sweep left ~25 note-text errors per stint range (truncations, placeholders, spliced notes, injected sweep commentary) and 32 keys one word early in T5's range; stints corrected all at the plate. Blind read: **77 findings, 73 applied**, 16 of them slips in OUR Latin note layer (digits, e.g. Saragossa 381/380, and mistranscribed words) fixed in `data/plate-notes/*.tsv`. Polarity recorded: 7 sites (7090), 0 (7089), 2 (7091). Merge layers: `src/english/{7089,7090,7091}/cruces.md`, beside the per-stint files (the builder publishes any `cruces*.md`).

**HELD (need a worn-plate read or second witness):** 0497B *manendam/manendum* · Pelagius *sibi/tibi* in the chunk-0011 note (col 519, p.265) · Dionysius's number "55 or 35" in Ep. XVIII's note · worn-glyph TEI patches 0545A *impletas* and 0575B *misimus* · 0606A *epicopis Apulis* left unmarked, inconsistent with the `[sic:]` on *fratrer* and *Cathaginensi* in the same range (one more plate read would settle it).
**RULINGS — ALL THREE DONE (Wilson, 2026-09-24), no change to the text:** (1) 7091 keeps TWO English texts per letter (Greek block from the Greek, Latin block from the Latin, each with its own anchors). (2) ἀρχιερεύς = "high priest". (3) 0570B *quae fidelia corda sustineant* = "what faithful hearts might endure" (printed subjunctive, no negative, no question mark added). Still owed: Wilson's OK to deploy.
**Loose ends:** `index-work` left 4 unparsed scripture aliases on 7089 (Lucae I,6; Lucae XVIII,16; Naum I,9; Naum. I. 9 sec. LXX) in `data/index/pl/7089.json`, recorded, not dropped. A `[cn:]` cannot hold `]`: note 574 d uses fullwidth ［ ］ (renders correctly).

**Next pilot:** *Ordo Romanus XIV* (Jacobus Caietanus, 8096, 61K w). Plan the plate read from the page count at chunking, per runbook 4a‴. The Phase-2 recipe that worked here: `data/briefs/7090-STINT-BRIEF.md` and `data/briefs/INNOCENT-BLIND-READ-BRIEF.md`; tell stints up front that English carries `[nt:]` only (no `[cn:]` copy), that the sweep's note texts and keys need checking at the plate, and to keep a NOTE-EDITS log so the TSV can be synced.
