...
- **Indexing-from-day-one DECIDED 2026-07-03 (Wilson's call: track from the start, no retroactive passes).** `scripts/index-work.mjs` runs after verify on every shipped work → `data/index/pl/<idno>.json`. Tracks facts only (citations/columns/heads — see CLAUDE.md rule 9); theme tags + source-name vocab deliberately deferred until a corpus slice exists to derive them from. Abbo indexed: 21 scripture (dual OSIS refKey + verbatim Migne display), 37 fontes, 54 heads, **89 headnotes** (in-text attribution segments before every « quote, la+en paired — the cross-volume citation layer, Wilson-requested), 0 unparsed.
- **Read-through session 2026-07-03 (Wilson approved: design, responsiveness, readability).** Landed: **`translation-style.md`** — scalable register rulebook; read-through flags capture construction CLASSES (pattern 1 partitive genitive, pattern 2 attribution headnotes as citations), each with a corpus sweep; every future translation prompt points here + anchor chunk (CLAUDE.md rule 5). **Scripture policy LOCKED**: translate what Migne prints, never substitute a conventional version (Douay-base rejected; divergences are the payload). **`content/editorial-method.md`** — reader-facing "On the text and the translation" (Migne = reprint of la Bigne→Maxima→Galland→Maurist lineage, no fresh collation; why scripture follows the page; small-type parentheticals are editorial apparatus on Migne's page, NOT the author's). **Design fixes**: modern-u enforced on all `[lang=la]` (heads had locl v's), vol-badge → true spine (shelf language), English anchors → right margin.
