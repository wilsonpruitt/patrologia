# Cruces — 11064 (Hugh of St Victor, Expositio in Hierarchiam Coelestem)

## Col 0938A (chunk 0005)
- **Reading (as printed):** `iterum restituens reparando, sicut poenis constituit creando`
- **Note:** `poenis` breaks the clean parallel *restituens reparando … constituit creando* ("restoring by repairing … as it established by creating") and is theologically odd (why "by punishments" in the act of creation?). Likely a corruption (perhaps for *prius* or *primis*). Rendered literally: "again restoring us by repairing, as it established us by punishments in creating."
...
- **Note:** `vel animum` is a corruption of *velaminum* ("of veils") — confirmed by Hugh's own gloss two clauses later, which quotes the lemma correctly as *nisi varietate sacrorum velaminum circumvelatum* ("veiled about by the variety of sacred veils"). The lemma as printed was rendered literally ("by the variety of sacred things, or the mind"); the gloss was rendered with the correct *velaminum* ("sacred veils").

## Col 0976A (chunk 0019)
- **Reading (as printed):** `... qui est in homine. » [0976A] [n: (I Cor. II)]. »`
- **Note:** Migne prints a redundant closing guillemet: the I Cor. 2:11 quotation ("Nemo hominum scit...") closes with `»`, then a page-break anchor and the citation note follow, and a second stray `»` appears after the note. Reproduced verbatim in the English (marker order preserved; the stray guillemet is Migne's typesetting artifact, not a second quotation boundary).
...
- **Note:** `[sed]` is a bracketed editorial supplement inside the quoted (Scotus) lemma. Reproduced in the English as "[but]" — bracket preserved, content translated.
...
- **Note:** `[adhuc immundae]` is a bracketed editorial insertion within the corrupt Scotus lemma (which is grammatically broken throughout this stretch). Reproduced in the English as "[still unclean]" — bracket preserved, content translated; surrounding clause rendered as literally as the garbled Latin allows.
...
> ⛔ **CORRECTED 2026-08-14 by the `[sic:]` force-quarantine sweep (`translation-style.md` 7a⁗). Two changes, and the second overturns the conjecture above.**
>
> **1. Both markers now carry their force in plain English beside them** — *a raging she-bear [sic: ur sam]* and *I will add [sic: Ad dam]*. The marker still isolates the defective run, spacing and all; what it may not do is take the sense with it. The withdrawal recorded above withdrew the sense, not merely the styling.
>
> **2. `Ad dam` is NOT *ad dam[m]am*, "to a doe."** It is ***Addam***, "I will add" — a first-person future verb, not a preposition plus a noun. **The work refutes its own conjecture:** Hugh glosses this very lemma at col. 0986D (chunk 0022), where Migne prints « *Addam vero,* » and Hugh expounds it in the first person — *praesumam quidem … qui et hoc divinis significationibus **adjiciam*** — and our own published English there already reads **"« But I will add, » I will indeed presume … I who add this too to the divine significations."** So our page said "I will add" at one column and "to a doe" at another, for the same word.
>
> That also fixes the syntax: *quod omnium vilius esse … visum est* is the **accusative object** of *addam*, not a second dative governed by *ad*. The English no longer reads "and to that which seemed…" but "that which seemed…", and the sentence has a main verb for the first time.
>
> This was two defects stacked — a swallowed verb *and* a wrong conjecture propagating a dative into the lemma — and neither was visible without reading the two columns together.
...
- **Note:** These are the edition's own bracketed alternative readings (Migne/source apparatus), not garbled type. The bracketed Latin alternates are preserved verbatim and untranslated in place, immediately after the translated primary reading (cf. the `[ *al.,* …]` variant convention).

## Col 0925A (chunk 0001) — bracketed editorial variant
- **Reading (as printed):** `Sed mundus medicum [modicum] despexit`
- **Note:** The edition's own bracketed alternative reading, not garbled type. Primary reading `medicum` ("the physician" — apt to the immediately following healing imagery: disease, medicine, the blind man's cure) is translated; the bracketed alternate `[modicum]` ("a little / of little account") is preserved verbatim and untranslated in place. Rendered "But the world despised the physician [modicum]."
...
- **Note:** `excelsum rationis` almost certainly a misprint for `excessum rationis` — the parallel clause reads `excessum sensibilitatis`, and the following gloss reads `Excessus enim rationis et sensibilitatis vel sursum fit, vel deorsum`. Left literal rather than emended: "irrationality, which signifies the loftiness of reason; and insensibility, which demonstrates the excess of sensibility."

## Col 0991B (chunk 0024) — dagger/lacuna in the Dionysian lemma
- **Reading (as printed):** `operari quod omnino praeter hostiarum + mysteria, aut sacras ordinationes`
- **Note:** Migne prints a cross/dagger (reproduced as `+`) between `hostiarum` and `mysteria`, marking a corruption/lacuna in the Latin of the quoted Dionysian text (the LITTERA). Marker reproduced verbatim in place: "to do anything at all apart from the + mysteries of the sacrificial victims, or the sacred ordinations."

## Col 0994C (chunk 0025) — garbled type
- **Reading (as printed):** `Scopas autem, id est directio vel destinatio, est qua tendit`
- **Note:** The transliterated Greek σκοπός is spelled `Scopas` here, though the same passage elsewhere gives `scopos` (nominative) and `scopon` (accusative). Apparent compositor's slip. Rendered as printed (*scopas*), not normalized.

## Col 1005B (chunk 0029) — garbled type in the Dionysian lemma
- **Reading (as printed):** `et de angelo ipsum Vesum confortante`
- **Note:** `Vesum` is a misprint for `Jesum` — the angel strengthening Jesus (cf. Luke 22:43); the very next clause names `ipse Jesus` explicitly. Rendered literally as *Vesum* within the italic LITTERA run, not emended.
...
- **Note:** The lemma prints two words `in opportunitate` ("in opportuneness/fitness"), whereas Hugh's own gloss reads the single word *inopportunitate* ("inopportuneness") and builds the sense on it (dimmer light in grosser matter = *inopportunitas*). The LITTERA was rendered literally as printed ("... in relation to the distributive habit of illumination in point of opportuneness"); the gloss was rendered with its *inopportunitas* ("the inopportuneness of the matters to be illuminated"). No emendation of the LITTERA Latin.
...
- **Note:** `una` in the LITTERA is almost certainly a corruption of *vana* — Hugh's gloss (1076A, chunk 0053) reads *vana* and explains it as *quae per se considerata ima apparent, et transitoria* ("things which, considered in themselves, appear lowly and transitory"). The LITTERA (chunk 0052) was rendered literally as printed ("turned toward none of the things seen as one"); the exposition (chunk 0053) follows its own *vana* ("seen as vain"). Neither Latin is emended.
...
- **Note:** `meditantibus` is a real Latin word ("meditating", abl. pl. participle of *meditor*), so per Pattern 7 it is rendered as printed: "just as it itself is turned to the likeness of God by the superiors meditating, so, with it mediating, the things that are lower are called back." The intended reading is *mediantibus* ("mediating"), required by the parallel clause immediately following, *ita mediante ipsa quae inferiora sunt … revocantur*, which prints correctly and is translated so. An earlier draft read the intended word in both places; that has been withdrawn. **Pattern 18 applied 2026-08-15 (withdrawn-net read):** the printed word is real and the English asserts a doctrine Hugh is not stating — mediation, not meditation, is the whole point of the clause, and its own parallel *ita mediante ipsa* is the contradicting neighbour. The page now reads `by the superiors meditating [cj: *meditantibus*; read *mediantibus*, "mediating"]`.
...
- **Note:** Migne opens the Dionysian lemma with `«` at *Hoc perhibetur* but prints no closing `»` before Hugh's gloss (*hoc scilicet, quod ...*) begins; a second `«` then opens the embedded Hosea/Jeremiah quotation (*Cognitionem Dei repulisti ...*), which alone closes with `»`. Reproduced verbatim: the guillemets are kept exactly where Migne set them (two opens, one close in this stretch), so Hugh's gloss visually falls inside the unclosed lemma as in the source. Not emended.

## Cols 1092B (chunk 0059) — authorial glosses bracketed inside a scripture quote
- **Reading (as printed):** `Dicentes enim se esse sapientes [ecce superbia] stulti facti sunt, et obscuratum est insipiens cor eorum [ecce error in non existentes deos]` (within the Rom. 1 quotation)
- **Note:** `[ecce superbia]` and `[ecce error in non existentes deos]` are Hugh's own interpretive asides (tying the Pauline quote to his two points — pride and error unto non-existent gods), interpolated in square brackets, NOT edition-apparatus variant readings (unlike *[modicum]*, *[visionem]*, *[forma]*, which are left untranslated). Because they are authorial prose content, they are TRANSLATED in place and kept in brackets: "[behold, pride]" and "[behold, the error unto non-existent gods]."
...
- **Note:** The edition's own bracketed alternative reading, not garbled type. Primary reading `revocans` ("recalling") is translated; the bracketed Latin alternate `[renovans]` ("renewing") is preserved verbatim and untranslated in place (cf. the `[ *al.,* …]` variant convention). Rendered "that which recalls [renovans] the intellectual furrows."
