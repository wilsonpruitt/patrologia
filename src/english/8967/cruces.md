# Cruces — 8967, *Glossa ordinaria*, **Liber Psalmorum** (PL 113, cols 0841A–1080B)

## BAND A — chunks 0000–0038, cols 0841A–0918C, Psalms I–XLIX

**Merged 2026-09-04 from eight stint files.** Bands B (chunks 0039–0076, Ps 51–100) and C
(0077–0105, Ps 101–150) are separate runs; **the work ships only when all three are merged.**

⛔ **The merge is not the concatenation.** §A below is the reconciliation — the conventions that
now bind Bands B and C, the drift that was found, and how each was settled. The eight stint files
follow **verbatim and unabridged** in §E; nothing in them has been cut.

---

# §A · WORK-WIDE CONVENTIONS — settled, and binding on Bands B and C

## A1. The tituli — ruled from the Glossa's own argument, not from preference

1. **`In finem` → "Unto the end."** ⛔ NOT "for the choirmaster" (the sense of the Hebrew
   *lamnatseach*). **The Glossa argues from *finis* meaning END**: `Finis duobus dicitur modis,
   quando aliquid deficit, vel quando perficitur`. A stint found further evidence at 0884B, where
   Migne writes `Non moriar jam in finem` in his own prose four lines from the titulus.
   **Verified across Band A: 26 occurrences, zero rivals.**
2. **`Diapsalma` / `Diapsalmus` → unanglicized and ROMAN, never italic, never "Selah".**
   ⚑ **This was the one true convention conflict in Band A, and the fault was the launch brief's,
   not a stint's.** The brief said "italic as Migne sets it" — and Migne sets it ROMAN. Two stints
   read the plate and overrode the brief; one obeyed it and italicised a single occurrence. They
   were not diverging from each other, they were following contradictory instructions. Settled on
   five plate reads (0849B, 0870D–0883A, 0887C, 0891D, incl. the masculine form); the single
   italic is reverted; the brief is corrected. **Italicising it also breaks the Latin/English
   span parity every stint checks — the error had a second, silent cost.**
3. **`Canticum graduum` → "A canticle of degrees", and `gradus` → degree throughout.** Load-
   bearing, not stylistic: the gloss numbers the gradual psalms as a series in its own voice
   (*Quartus gradus*, *Septimus gradus*, *Decimus gradus*). ⚑ **Band A contains none of these** —
   they begin at Ps 119, in Band C. The ruling is recorded here so Band C inherits it.
4. **`Psalmus David` and `Psalmus ipsi David` are different lemmata and stay different.** Several
   carry a `⚑` in the lemma brief because Migne's titulus and the Clementine's differ; three
   stints fired `[var:]` on such tituli, consistently.

## A2. Psalm numbering — Vulgate, and the one apparent exception is not one

**Vulgate/LXX numbering governs the whole book. Never convert a psalm number.** Chunk 0010 carries
**two heads both numbered X** — `PSALMUS X.-- Secundum Hebraeos.` (the Hebrew psalm 10, which our
Clementine holds as the tail of Ps 9, its `VERS.` restarted at 1) and `PSALMUS X.` (the Vulgate
Ps 10). Migne glosses the passage twice under one numeral, and that is his text: **nothing was
renumbered, merged or marked.** The stint collated all nine `⚑ Ps 9:2x` spans individually and
every one agreed word for word.

⛔ **The natural inference — that the Glossa follows the Hebrew division and runs one ahead
thereafter — is false, and was tested rather than assumed**: first-lemma probes for every psalm
head in the work score **138 same-number / 0 offset / 3 undecided**.

## A3. Heads

`PROTHEMATA IN PSALTERIUM.` → **`PROTHEMATA ON THE PSALTER.`** · `PSALMUS PRIMUS.` → **`PSALM
ONE.`** but `PSALMUS II.` → **`PSALM II.`** — **Migne spells the first out and numbers the rest,
and his inconsistency is followed, not regularised.** Roman numerals stay Roman.

## A4. The vocabulary axes — measured across all 39 chunks, not asserted

A mechanical sweep of the merged Band A English:
...
| `turba`/`multitudo` | crowd 0 · multitude 1 — **the instrument was checked: this is a Psalter, and the Gospel crowds are simply absent.** Not evidence of drift. |

⚑ **The archaism check needed a refinement, and it is a real finding for Bands B and C.** The
settled test for whether `-eth`/`saith` is licensed is **italic membership** — archaism is
permitted only inside quoted scripture. A sweep flagged one `saith` as gloss-voice. It is not:
it stands inside **« guillemets »**, quoting Isa 45:1. **Quoted scripture in this work is marked
...
# §B · WHAT THE PLATE DID TO THIS BAND

**46 columns read**, every marker standing on a column somebody opened (`plate-gate` OK).
**Zero foot-of-page conjecture apparatus** on any page read, in either layer.

⭐⭐ **The measured case for the plate gate, and it is the strongest this project has produced.**
- One stint had **six `[sic:]` candidates and the plate withdrew five.** Every withdrawn one read
  as a confident Migne defect **from the Latin alone**.
- Another had the exact mirror: it **declined** `In sua in virtute` @0890A reasoning that a doubled
  short word is the classic transcriber artifact — and at 500 dpi **Migne sets it plainly.** A
  `[d: in]` now stands there.

**Neither "trust the text" nor "suspect the transcriber" is a rule. Only the leaf is.**

⛔ **The mod-4 clipping rule the launch brief once gave is WITHDRAWN.** It said a left-hand column
numbered 1 (mod 4) is clipped at the inner margin. Refuted: **841, 849, 893, 897, 1045 clipped
against 853, 857, 861, 865, 869, 877, 881, 889, 901, 909 clean.** A coin flip. The original claim
rested on four consecutive leaves plus one — a single physical gathering — so the congruence was
an artifact of contiguity. **Clipping is real and sporadic; open the column, never infer.** Its
failure mode runs where nobody audits: applied blind it silently *withholds* good markers, and it
had blocked three before a stint looked.

# §C · THE PATCH PASS — what changed under this band, and why

**39 TEI patches applied in one pass** after the band's translation was complete
(`data/tei-patches/8967.json`; queue and classes in `data/briefs/8967-PENDING-TEI-PATCHES.md`).
Our Latin is Corpus Corporum's transcription, and across the ~46 columns read it departs from
Migne at **roughly 1.5 sites per column** — consistent with 9003's independent collation of the
sibling PL 114 Psalter at ~3 per page. **The class runs in three directions:**

1. **CC moves Migne ONTO the Clementine.** `Dominus conservet` (0903D), `audiebam` (0899B),
   `populis` (0895A), `Multiplicati sunt super numerum` (0902C), `sordescat` (0847A). **Invisible
   to any Vulgate collation by construction** — the divergence is erased before an instrument looks.
2. **CC REPAIRS Migne's real defects, destroying markers we are entitled to fire.** `Nocere`
   (0908C, restored — `[sic: *Nocere*]` now stands), `iniquitetem` (0910C), and two erased
   dittographies (`etsi. Etsi` → `etc.` @0887B; `doc-/ctus`).
3. **Our Latin carried words the plate lacks.** 0904A, where CC replaced Migne's `quasi,` with
   **`sed Deus`** and inserted `in`. Verified on the leaf before patching.

**Four English twins changed with the Latin**: 0908C (`[sic: *Nocere*]` fired), 0904A, 0866D
("or the fat" → "the fat"), 0902C ("above number" → "in number").

⛔ **Three reported sites were REFUSED, and each refusal is a rule:**
- **`locutus` → `locutos`** — at 900 dpi the glyph is ambiguous, and `locutus` (perfect deponent,
  "and afterwards I myself spoke") is correct Latin where `locutos` is a solecism. **A patch needs
  positive evidence.**
- ⭐ **`possit` → `rossit`, and its whole class.** **The plate's broken sorts are not Migne's
  errors.** A damaged `p` that images as `r` is the condition of *this copy's inking*, not a
  reading of the edition. **Compositor errors (wrong sorts SET) are restored; ink damage is not** —
  that line is why `iniquitetem` and `Nocere` are in and `rossit` is out.
- **`Desiderabilia` → `Desiderabillia`** — deferred: two occurrences stand in col 0871D and the
  report does not say which. Re-read the leaf; do not guess.

⚑ **And one site was confirmed against the editor's own doubt.** `Multiplicati sunt super numerum`
looked wrong to challenge, because the gloss beneath repeats "Super numerum". The plate settles it:
the lemma reads `numerum` and the gloss's `Super numerum` is the gloss **expanding** its lemma,
which is how this commentary works throughout.

## C1. Patched in the Latin, deliberately NOT marked
**0903D `Dominus` → `Dominum conservet`.** The English is unchanged. Convention 3 renders a
fragmentary lemma as the phrase it stands for (Ps 40:3), and **no `[var:]` is owed**: the gloss
does not argue from the word, and `Dominum`/`Dominus` is exactly the letter-level corruption the
settled single-word `[var:]` rule excludes.

## C2. Markers the patch pass UNLOCKED, for a later pass
- **`Seb isti` @0876B** — our twin read `Sed isti` and did not construe; the plate reads `Seb
  isti`; the conjecture is `Sedisti` (Prov 23:1–2), supplying exactly the missing verb and person.
  **No marker could reach it before the patch**: `[sic:]` may wrap only type the twin carries and
  `[cj:]` needs the printed word present. **The patch is a precondition for the apparatus, not a
  cleanup after it.**
- **0902C** a `[var:]` against Ps 39:6 is now available on the restored lemma.

# §D · STILL OWED BEFORE THE WORK SHIPS

1. **The blind polarity read** on all 39 chunks — required, and it is a reading pass, not a grep.
2. **Bands B and C**, then a three-band merge of this file.
3. **Gallica, three sites this copy cannot answer**: `quae non est nisi [inquieta] mente` @0885B
   (`f443`) · `enumerantur`/`emunerantur` @0898D (`f449`) · `excrescentibus pilis` @0905A (`f453`).
4. **One `In Hebraeo` clause unread.** Two of three are closed: 0868A prints `(quod in Hebraeo)`
   with no Hebrew type at all, and 0902B's `In Hebraeol` was CC debris for `In Hebraeo`.
5. The `gens tenebrarum` ruling (§A5).

**Band A marker totals: 37 `[var:]` · 6 `[sic:]` · 2 `[cj:]` · 1 `[d:]`.**

---

# §E · THE EIGHT STINT FILES, VERBATIM
...
## §E.0000 — stint file `cruces-0000.md`, verbatim
...
## §E.0005 — stint file `cruces-0005.md`, verbatim
...
## §E.0010 — stint file `cruces-0010.md`, verbatim
...
## §E.0015 — stint file `cruces-0015.md`, verbatim
...
## §E.0020 — stint file `cruces-0020.md`, verbatim
...
## §E.0025 — stint file `cruces-0025.md`, verbatim
...
## §E.0030 — stint file `cruces-0030.md`, verbatim
...
## §E.0035 — stint file `cruces-0035.md`, verbatim
...
# §F · BAND B — THE MERGE (chunks 0039–0076, cols 0918C–1011B, Psalms L–C)

Eight Opus stints, 38 chunks, 39,328 Latin words. **`verify-english` clean, `plate-gate` OK,
`patch-audit` OK.** This section is the reconciliation, not the concatenation; the eight stint files
stand verbatim in §G.

## F1. ⭐ EIGHT FOR EIGHT ON THE SPAN COUNT

384 · 294 · 373 · 367 · 352 · 272 · 302 · 258 — every stint's own count of the italic spans in its
Latin matched its lemma brief **exactly**. That count is the only detector a mis-split brief has
(`verify-english` cannot see one), so the Band B split is sound everywhere it was tested. Two stints
also checked that the brief's first and last entries matched their range's first and last spans.

## F2. ⛔⛔ THE HEADLINE, AND IT CORRECTS WHAT THE BAND LOOKED LIKE BEFORE THE PATCH PASS

Before the patch pass Band B stood at **106 `[var:]` against 4 `[sic:]`**, and that ratio was read —
by me, in this session — as evidence that the defects in these columns are overwhelmingly Corpus
Corporum's rather than Migne's. **That reading was wrong, and the error is instructive.**

**The `[sic:]` count was suppressed by patch ordering, not by Migne's cleanliness.** A `[sic:]` may
wrap only type our twin carries; CC had already repaired dozens of Migne's real defects, so the
markers were unfireable while the twin disagreed with the plate. One stint reported **zero `[sic:]`
on a range containing twelve** Migne defects, and another found that its twin carried **none** of the
six standing on its leaves.

**After the patch pass restored them, Band B stands at:**
...
⭐ **The lesson is general and belongs to the series: on a work whose twin has been silently repaired,
NO MARKER COUNT MEANS ANYTHING UNTIL THE PATCH PASS HAS RUN.** Quoting one before is quoting the
digitization's opinion of Migne, not Migne.

## F3. THE PLATE — 90 columns, none of which had been opened when the band began

Every stint read its own leaves; `plate-gate` refuses a marker on a column nobody opened, and it
passes work-wide. What the leaf did that no other instrument could:

- **It withdrew markers.** Six candidates in Band A's own reckoning, and in Band B at least sixteen
  more, every one of which read as a confident Migne defect **from the Latin alone**. One stint had
  six `[sic:]` candidates and the plate took five.
- **It reversed the direction three times in one range** — two candidates withdrawn as ours,
  `intrepretatur` confirmed as Migne's when it read like the classic transcriber artifact.
- ⭐ **It stopped a false `[var:]` against Migne.** `solennitatis` @0960A looked like a divergence
  from the Clementine; the plate shows Migne prints `solemnitatis`, **agreeing** with it. CC had
  de-diphthonged him and manufactured the appearance of a divergence.

### ⚠ A SECOND IMAGING HAZARD, MEASURED — italic `e` images as `c`
Found by the 0044–0048 stint (`Ostcndisti`, `omncs`, `Vallcs`), briefed into wave 2, and it then
withdrew **eleven** false accusations against Migne across the band (`omncm`, `opcrantis`, `Zcb`,
`Eccc`, `undc`, `verbornm`, `agom`, `nocent` among them). **Without a same-letter control on the same
line, this one hazard would have manufactured a defect class that does not exist.** It belongs beside
the `h`/`b` warning — which also fired **in reverse** this band, our TEI carrying `biatu` where the
plate reads `hiatu`.

⛔ **The turned-sort pair `Viuo` and `Clamabuut` is deliberately NOT in that class**: an `n` cannot
become a clean bottom-arched `u` by failing to ink. The distinction is what makes the hazard rule
usable rather than a licence to decline everything.

### The mod-4 clipping rule stays withdrawn, and is now refuted 22 more times
Band A drew "column ≡ 1 (mod 4) is clipped" from four consecutive leaves. Band B opened ~90 columns
and found **exactly one clipped column in the whole band, 0981** — and the marker resting in its
margin was **WITHHELD and logged for Gallica, not fired.** That is the rule's real failure mode
(silent withholding) being made visible instead of being allowed to happen.

## F4. OUR LATIN IS NOT MIGNE'S — 149 patches over 73 columns

~1.4 departures per collated column, agreeing with Band A and with 9003's independent collation of
the sibling PL 114 Psalter. The class runs three ways, all three found here:

1. **CC moves Migne ONTO the Clementine** — invisible to any Vulgate collation *by construction*.
   ⭐⭐ `conculcabit`→`conculcabis` @1000B is the one to remember: the gloss's very next words are
   *Serpentem calcat Ecclesia*, **the third person the whole exposition is built on.**
   `Audivit`→`Audivi` @1007B erased a first/third-person divergence before any instrument looked.
2. **CC REPAIRS Migne's defects**, destroying markers we are entitled to fire — including a
   **compositor eye-skip** @0991C, where CC reconstructed the sense Migne lost, so nothing in our
   twin could ever have shown the plate was broken.
3. **CC MANUFACTURES defects and readings.** ⭐ @0942A it INSERTS `Sina` into a lemma whose whole
   point is that `Sina` is absent — Migne's next gloss reads *Quidam libri non habent mons, sed a
   facie Dei Sina Israel* — **making Migne argue against a text he did not print.** And @0999D/1000A
   it manufactured a dittography at the column turn where Migne breaks `Ideo-|que`; **no `[d:]` is
   owed, because the doubling is ours.**

## F5. WHAT THE PATCH PASS ITSELF FOUND — `scripts/patch-audit.mjs`

`chunk-work.mjs` refuses a `find` that matches zero or many times. **That is uniqueness, and
uniqueness is not location.** The new audit resolves each patch's EDIT offset to its enclosing
`<pb/>` and compares with the declared column. On its first run it found:

- **24 of Band A's 43 applied patches had NO column recorded at all**, so the check had never been
  possible on more than half the file.
- **One Band A patch was filed at the wrong column** (`Gen. III`, filed 0855C, lands 0853D) — the
  9003 failure recurring. The repair was aimed right; the RECORD was wrong, which would have sent
  anyone re-reading that leaf to the wrong page.
- **Four Band B sites were one BAND letter off**, and a patch pair at 0928B **would have deadlocked
  the queue** — one edit fell inside the other's find string, so the second would have matched zero
  times and aborted chunking outright.

⭐ **The ordering rule was then vindicated by measurement:** re-chunking with all 192 patches left
**Band A's 39 Latin chunks and Band C's 29 byte-identical**, all 38 of Band B's changed, and no
chunk boundary moved — so Band A's filed English was untouched and Band C's lemma briefs did not
need regenerating.

## F6. CONVENTIONS — inherited, confirmed, and the exceptions taken

- **§A2 `Diapsalma` ROMAN** — confirmed by **twelve more plate reads** across the band.
- **§A4 `Psalmus David` ≠ `Psalmus ipsi David`** — ⭐ now evidenced from Migne's own plate rather than
  merely obeyed: at **1010A he prints `Psalmus ipsi David` exactly**, four columns from the
  `Psalmus David` that took a `[var:]`. He sets both forms.
- **`In finem` → "Unto the end."** held throughout. ⚑ One recorded non-engagement: @0976D Migne does
  not print `In finem` at that titulus at all (`Intellectus pro his qui commutabuntur`), so the
  ruling is not engaged there.
- **`usque ad`** — three stints independently took exceptions, and **Wilson's ruling of 2026-09-04
  (§A7) settles the class**: the `etc.` splice test replaces italic membership. Re-audited across the
  band afterwards: **274 formula, 18 non-formula, and every English rendering is consistent with the
  ruling.** The two `as far as` outside a splice are genuine sense-renderings (*spread as far as the
  ends of the earth*).
- **`gens tenebrarum`** does not occur anywhere in Band B; ruled by Wilson on Band A's single site as
  **"people of darkness"** (§A5).
- **Convention 3 exception, named:** `Deus quis similis?` @0982C — expanding the stump would silently
  decide the *est*/*erit* question the gloss exists to argue.

## F7. ⛔ STILL OWED

1. **The blind polarity read has NOT run on Band B.** Band A's found six of our own defects, four
   invisible to every other check, and one stood in a column the cruces had already logged. **A
   logged column is not a checked column.**
2. **Five reported sites remain unpatched**, each because the report itself declined or could not
   rule: `Nafrubimus` @0962B (the letter would not settle as `f` or `t` at 1200 dpi, and **a patch
   must not guess a letter any more than a marker may**), `Malivolenti :` @0929A, 0926B's
   self-contradictory report, `res alienas`/`res aliena` @0971D, and `(Rom. IX)`→`(Rom. IV)` @0971A —
   where **CC CORRECTED Migne**, so restoring it needs a `citation-corrections.json` entry landed with
   it (⛔ column written WITHOUT its leading zero, or it matches nothing and still reports 0 unparsed).
3. **Five Gallica sites**, one IIIF request each: `f443`, `f449`, `f453` (Band A), **0932B**
   (`Misericordia`/`Misericordiam`, in col 0932's over-inked outer margin) and **0981C**
   (`filiis`/`filii Excelsi omnes`, in the band's one clipped column).
4. **Residual prose punctuation** listed by three stints as "recorded, not argued" is not applied;
   the sense-bearing and lemma-internal marks are.
5. **Band C** — chunks 0077–0105, 29 chunks, 28,516 words. Its briefs are current and unaffected by
   the patch pass. It carries the gradual psalms, and it inherits §A6 (the plate where read), §A7
   (the `etc.` splice test) and the two imaging hazards.

⛔ **Nothing ships until all three bands are merged.** Nothing here is pushed or deployed.
...
Nothing below is abridged. Each stint's own reasoning, declined list and checks stand as written —
including the candidates it raised and rejected, which are the measured instrument.


## §G.0039 — stint file `cruces-0039.md`, verbatim
...
## §G.0044 — stint file `cruces-0044.md`, verbatim
...
## §G.0049 — stint file `cruces-0049.md`, verbatim
...
## §G.0054 — stint file `cruces-0054.md`, verbatim
...
## §G.0059 — stint file `cruces-0059.md`, verbatim
...
## §G.0064 — stint file `cruces-0064.md`, verbatim
...
## §G.0069 — stint file `cruces-0069.md`, verbatim
...
## §G.0073 — stint file `cruces-0073.md`, verbatim
